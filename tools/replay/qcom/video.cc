...
check_extradata_payload(enum msm_vidc_extradata_type type, void *data, int size)
{
	switch (type) {
	case MSM_VIDC_EXTRADATA_OUTPUT_CROP: {
		struct msm_vidc_output_crop_payload *payload = (struct msm_vidc_output_crop_payload *)data;
	...
	case MSM_VIDC_EXTRADATA_ASPECT_RATIO: {
		struct msm_vidc_aspect_ratio_payload *payload = (struct msm_vidc_aspect_ratio_payload *)data;
	...
	case MSM_VIDC_EXTRADATA_INTERLACE_VIDEO: {
		struct msm_vidc_interlace_payload *payload = (struct msm_vidc_interlace_payload *)data;
		...
		dbg("extradata: %s format=%s color_format=%s",
		    extradata_type_to_string(type),
		    extradata_interlace_format_to_string((enum msm_vidc_interlace_type)payload->format),
		    extradata_interlace_color_format_to_string(payload->color_format));
		break;
	}

	case MSM_VIDC_EXTRADATA_MASTERING_DISPLAY_COLOUR_SEI: {
		struct msm_vidc_mastering_display_colour_sei_payload *payload = (struct msm_vidc_mastering_display_colour_sei_payload *)data;
	...
	case MSM_VIDC_EXTRADATA_FRAME_RATE: {
		struct msm_vidc_framerate_payload *payload = (struct msm_vidc_framerate_payload *)data;
...
extradata_header_is_valid(const struct msm_vidc_extradata_header *hdr, int size)
{
	...
	while (left > sizeof (*hdr) && left >= hdr->size &&
		...
		if (hdr->type == MSM_VIDC_EXTRADATA_INDEX) {
			struct msm_vidc_extradata_index *payload = (struct msm_vidc_extradata_index *)(void *)hdr->data;
			ret = check_extradata_payload((enum msm_vidc_extradata_type)payload->type,
    (void *)((char *)hdr->data + sizeof (hdr->type)),
    hdr->data_size - sizeof (hdr->type));
		} else {
			ret = check_extradata_payload((enum msm_vidc_extradata_type)hdr->type,
					      (void *)hdr->data,
					      hdr->data_size);
		}
		...
		left -= hdr->size;
		hdr = (const struct msm_vidc_extradata_header *)((const char *)hdr + hdr->size);
	}
	...
	while (hdr && hdr->type != MSM_VIDC_EXTRADATA_NONE) {
		...
		if (hdr->type == MSM_VIDC_EXTRADATA_INDEX) {
			...
			if (type == (int)payload->type)
				return (void *)((char *)hdr->data + sizeof (hdr->type));
		}
			...
			uint32_t flags, struct timeval timestamp)
{
	...
	if (ioctl(vid->fd, VIDIOC_QBUF, &buf) < 0) {
	  ...
	  //   buf_type_to_string((enum v4l2_buf_type)buf.type), buf.index);
		return -1;
	}

	// dbg("%s: queued buffer %d (flags:%08x:%s, bytesused:%d, ts: %ld.%06lu), %d/%d queued", buf_type_to_string((enum v4l2_buf_type)buf.type), buf.index, buf.flags, buf_flags_to_string(buf.flags), buf.m.planes[0].bytesused, buf.timestamp.tv_sec, buf.timestamp.tv_usec, video_count_output_queued_bufs(vid), vid->out_buf_cnt);
...
int video_queue_buf_cap(struct instance *i, int n)
{
	...
	// dbg("%s: queued buffer %d, %d/%d queued", buf_type_to_string((enum v4l2_buf_type)buf.type),
  //   buf.index, video_count_capture_queued_bufs(vid), vid->cap_buf_cnt);
	//static int frame_cnt = 0;
	//dbg("put %d empty frames into the capture queue (Contains blank frames for writing)", frame_cnt);
	//frame_cnt++;
...
static int video_dequeue_buf(struct instance *i, struct v4l2_buffer *buf)
{
    ...
    // buf_type_to_string((enum v4l2_buf_type)buf->type));
		return -errno;
	}
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE:
		...
		// dbg("%s: dequeued buffer %d (flags:%08x:%s, bytesused:%d, ts: %ld.%06lu), %d/%d queued", buf_type_to_string((enum v4l2_buf_type)buf->type), buf->index, buf->flags, buf_flags_to_string(buf->flags), buf->m.planes[0].bytesused, buf->timestamp.tv_sec, buf->timestamp.tv_usec, video_count_capture_queued_bufs(vid), vid->cap_buf_cnt);
		break;
	}
			  ...
			  struct msm_vidc_extradata_header **extradata)
{
	...
	*n = buf.index;
	...
	if (extradata_addr)
		extradata_valid = extradata_header_is_valid((const struct msm_vidc_extradata_header *)extradata_addr, vid->extradata_size);

	if (extradata)
		*extradata = extradata_valid ? (struct msm_vidc_extradata_header *)extradata_addr : NULL;
...
int setup_extradata(struct instance *i, int index, int size)
{
	...
	if (vid->extradata_ion_fd < 0) {
		...
		for (int n = 0; n < MAX_CAP_BUF; n++) {
			vid->extradata_off[n] = off;
			vid->extradata_addr[n] = (char *)vid->extradata_ion_addr + off;
			off += size;
		}
	}
...
int video_setup_capture(struct instance *i, int num_buffers, int w, int h)
{
	...
	if (ioctl(vid->fd, VIDIOC_S_FMT, &fmt) < 0) {
		...
		//     buf_type_to_string((enum v4l2_buf_type)fmt.type), w, h);
		return -1;
	}
    ...
    pix->num_planes, v4l2_field_to_string((enum v4l2_field)pix->field),
    v4l2_colorspace_to_string((enum v4l2_colorspace)pix->colorspace), pix->flags);
		       ...
		       unsigned int size, int count)
{
	...
	for (n = 0; n < vid->out_buf_cnt; n++) {
		...
		vid->out_buf_addr[n] = (char *)buf_addr + vid->out_buf_off[n];
...
int video_subscribe_event(struct instance *i, int e_type)
{
...
// V4L2 stubs at the very top
#ifndef V4L2_QCOM_CMD_FLUSH_CAPTURE
#define V4L2_QCOM_CMD_FLUSH_CAPTURE 0x08000001
#endif
#ifndef V4L2_QCOM_CMD_FLUSH_OUTPUT
#define V4L2_QCOM_CMD_FLUSH_OUTPUT 0x08000002
#endif
#ifndef V4L2_QCOM_CMD_FLUSH
#define V4L2_QCOM_CMD_FLUSH 0x08000003
#endif
#ifndef V4L2_PIX_FMT_NV12_TP10_UBWC
#define V4L2_PIX_FMT_NV12_TP10_UBWC 0x01000000 // placeholder
#endif
#ifndef V4L2_PIX_FMT_NV12_UBWC
#define V4L2_PIX_FMT_NV12_UBWC 0x02000000 // placeholder
#endif
